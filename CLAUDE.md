...
6. **品質チェックの再実施**: レビュー指摘対応が複数ラウンドに及んだ場合、最終コミット前に品質チェックを再実施すること
   - **必須条件**: 以下のいずれかに該当する場合は再実施必須
     - エラーハンドリングの大幅変更
     - 複数ファイルにまたがる修正
     - ロジックの変更を伴う修正（単純な文言修正・フォーマット修正は除く）
   - **実施内容**: テスト実行（test-runner）、コードレビュー（code-reviewer）、ドキュメントレビュー（doc-reviewer）を初回実装完了時と同じ基準で実施
   - **根拠**: 初回チェック通過 ≠ 最終品質保証。修正が入るたびに品質は変動する（参考: `docs/retro/auto-progress.md` 教訓5）
