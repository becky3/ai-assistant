...
5. **ストーリーテラーにチーム始動を通知**: 「チーム始動した」と伝える
6. **ターンを終了してプロンプトを返す**: メンバーのメッセージは自動配信されるため、TaskListのポーリングでターンを占有しない
7. **メンバーからメッセージが届いたら即座にユーザーに共有**: まとめず、届いた順に引用して伝える
...
- **必須メンバー構成**: チームには以下を必ず含める
  ...
  - **ストーリーテラー（1名）**: プロセス全体を俯瞰し、問題を早期発見する語り部。**各タスク完了時・品質チェック時・PR作成時に必ず状況を語る**（順調時も存在感を示す）
- **リーダーの報告**:
