...
**簡単な起動方法**: 「**チームで開発して**」と指示するだけで、キャラクターテーマがランダムに選択され、**リーダーを含む全メンバー**がそのキャラクターとして開発を進めます。

**キャラクターテーマ**: アニメ・漫画・ゲームのキャラクターをチームメンバーにして、楽しく開発を進めることができます。Claudeの知識から毎回ランダムにテーマが選ばれます。

- 特定テーマの指定: 特定の作品やグループ名を指示することも可能

**テーマ履歴**: 過去に使用したテーマは `~/.claude/team-theme-history.json` に保存され、ランダム選択時に直近10件との重複を避ける参考として使用される（完全排除ではなく多様性向上のためのガイダンス）

**キャラクター演出の必須ルール**（詳細は `docs/specs/agent-teams.md` 参照）:

- **リーダーもキャラクターになる**: リーダーは素の状態で指示を出すのではなく、テーマに沿ったキャラクター（例: 鬼滅の刃なら煉獄さんや冨岡さん）として振る舞う
...
- **メンバーの発言は逐次引用してユーザーに共有する**（まとめだけでなく生の発言を見せる）
- メンバーには細かく分割して報告させる（長い意見を一気に送らず、セクションごとに分ける）
