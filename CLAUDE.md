...
6. **複数ラウンド時の再品質チェック**: レビュー指摘対応が複数ラウンドに及んだ場合、最終コミット前に品質チェック（test-runner / code-reviewer / doc-reviewer）を再実施すること
   - 初回の品質チェック通過後でも、修正を重ねるたびにリグレッションが入る可能性がある
   - 特にエラーハンドリングの大幅変更や、複数ファイルにまたがる修正の場合は必須
   - 参考: `docs/retro/auto-progress.md`「教訓5: レビュー指摘対応後の品質チェック漏れ」
