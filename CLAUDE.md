...
- **リーダー管理専任**: リーダーは管理・調整・判断に専念し、実装・テスト・レビュー等の作業は行わない。作業が必要な場合は既存メンバーに割り振るか新メンバーをspawnする。Edit / Write ツールの使用は原則禁止（例外: dashboard.md の更新、CLAUDE.md 等の運用ルール更新、チーム管理に直接必要なファイル操作のみ）。全メンバーがブロックされている場合のみ、リーダーが一時的に作業を代行可
