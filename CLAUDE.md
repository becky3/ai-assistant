...
- **品質担当必須**: チームには以下のサブエージェントを使う担当を必ず1名以上含める
...
**サブエージェント vs チームの使い分け**:

- **サブエージェント**: 1つの専門タスクを単発で委譲する場合
  - テスト実行だけ → test-runner
  - コードレビューだけ → code-reviewer
  - ドキュメントレビューだけ → doc-reviewer

- **エージェントチーム**: 複数の役割で並行・協調作業する場合
  - 複数ファイルにまたがる実装（実装者 + テスト担当 + レビュー担当）
  - 調査 + 実装 + テストを同時進行
  - 複数視点での検証・バグ調査
