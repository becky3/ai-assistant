...
5. **複数ラウンド時の再品質チェック**: レビュー指摘対応が複数ラウンドに及んだ場合、最終コミット前に品質チェックを再実施すること
   - **必須条件**: エラーハンドリングの大幅変更、複数ファイルにまたがる修正、ロジックの書き換えがあった場合
   - **実施内容**: test-runner（テスト実行）+ code-reviewer（コードレビュー）+ doc-reviewer（ドキュメント変更がある場合）
   - **背景**: 初回チェック通過後も修正を重ねるたびにリグレッションが入る可能性がある（教訓: PR #270 Phase 1）
6. **対応コメント投稿**: PRに対応状況を説明するコメントを投稿する（`gh pr comment`）
