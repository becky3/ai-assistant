...
ユーザーが「**チームで開発して**」と指示するだけで、ランダムに選択されたテーマのキャラクターチームが編成され、開発が始まる。

### テーマ定義ファイル

テーマは `.claude/team-themes/` ディレクトリに Markdown ファイルとして定義される。

```
.claude/team-themes/
├── README.md           # テーマシステムの説明
├── sos-brigade.md      # SOS団（涼宮ハルヒの憂鬱）
├── ff7-avalanche.md    # アバランチ（FINAL FANTASY VII）
├── spy-family.md       # フォージャー家（SPY×FAMILY）
├── bocchi-the-rock.md  # 結束バンド（ぼっち・ざ・ろっく！）
└── frieren.md          # フリーレン一行（葬送のフリーレン）
```
...
| セクション | 内容 |
|------------|------|
| **作品設定** | 世界観、背景、チームの成り立ち |
| **チームメンバー** | ロール、キャラクター名、性格・特徴、担当 |
| **目標の意味付け** | 開発タスクを作品の物語に沿った形で解釈する方法 |
| **会話スタイル** | 各キャラクターの話し方、口調、特徴的なセリフ |
| **チーム運営の特徴** | チームとしてのダイナミクス、特徴的な進め方 |

### 目標の意味付け例

開発タスクを作品の物語に沿って解釈することで、キャラクターが自然に行動できる:

| 開発タスク | 物語での意味付け例 |
|------------|-------------------|
| バグ修正 | 敵の襲撃、トラブル対処、閉鎖空間の発生 |
...
1. `.claude/team-themes/` に新しい Markdown ファイルを作成
2. README.md のテンプレートに沿って内容を記述
3. 4人のチームメンバー（リーダー、仕様担当、実装担当、品質担当）を定義
4. 各キャラクターの会話スタイルを具体的なセリフで例示

### テーマ選択の仕組み

1. ユーザーが「チームで開発して」と指示
2. `.claude/team-themes/` からランダムにテーマを選択
3. 選択されたテーマを読み込み、チームを構築
4. キャラクターの性格・会話スタイルでコミュニケーション
...
>
> **キャラクターテーマ使用時**: 上記の技術ロール（仕様担当、実装担当、品質担当）はそのまま維持され、キャラクターの性格と会話スタイルが適用される。
...
- [ ] AC6: 「チームで開発して」でキャラクターテーマがランダム選択される
- [ ] AC7: 各キャラクターが設定された会話スタイルでコミュニケーションする
...
| `.claude/team-themes/*.md` | キャラクターテーマ定義 |
