...
チームメンバーからの報告はリーダーに届くが、**ユーザーにはリーダーの発言しか見えない**。そのため、リーダーは以下のルールを守ること:

1. **メンバーの発言はそのまま引用してユーザーに共有する**
   - まとめだけでなく、生の発言を見せる
   - メンバーの意見や分析の詳細がユーザーに伝わるようにする

2. **メンバーには細かく分割して報告させる**
   - 長い意見を一気に送らず、セクションごとに分ける
   - 例: 「DB設計について」→「コマンド体系について」→「移行設計について」と分割
   - これによりユーザーへの逐次共有がしやすくなる

3. **メンバーからメッセージが届いたら、すぐにそのまま引用して共有**
   - リーダーのまとめを待たず、届いた順に共有する
   - ユーザーがリアルタイムで議論の流れを追えるようにする

### チーム作成時の指示

チームを作成する際、メンバーへの初期指示に以下を含める:

```
報告は細かく分割して送ってください。
- 長い意見は一気に送らず、セクションごとに分けて送る
- 例: 「まず結論を送ります」→「次に理由1について」→「理由2について」
これはユーザーへの逐次共有のためです。
```
