...
1. **ストーリーテラーにチーム始動を通知**: 「チーム始動した」と伝える
2. **ターンを終了してプロンプトを返す**: ユーザーにプロンプトを返し、メンバーからのメッセージを受信可能にする
3. **メンバーからメッセージが届いたら即座にユーザーに共有**: まとめず、届いた順に引用して伝える

**重要**: メンバーからのメッセージは「リーダーのターンが終わった後」に配信される。TaskListのポーリングでターンを占有し続けると、メンバーのメッセージがキューに溜まり、最後にまとめて届いてしまう。

```
❌ NG: メンバースポーン後に TaskList をポーリングし続ける
   → ターンが終わらず、メンバーのメッセージが届かない

✅ OK: メンバースポーン後にプロンプトを返す
   → メンバーのメッセージが自動配信される
   → 届いたメッセージを即座にユーザーに共有
```
