...
- **起動するプロセスのライフサイクルは bot から独立**: bot 停止時に remote-control プロセスは道連れにしない（外出先ユーザーの作業中断を避けるため）。実装上は OS のデタッチ起動オプションに加え、bot 終了時の `cleanup_children()` の除外リストに起動済み remote-control プロセスの PID を渡すことで保護する
...
その他のエラー（claude 未インストール / パス不在 / タイムアウト等）は、原因を特定しやすい簡潔なメッセージを返す。公開チャンネルでの実行を想定し、Slack 返信にはシークレット値・絶対パス全文を含めない。

タイムアウト時などにログ参照先を案内する場合も、Slack 返信に含めてよいのは **ログファイル名のみ**（絶対パス・相対パスを含まない `<session_name>.log` 形式）とする。**絶対パスは管理者向け記録としてサーバーログにのみ出力する**。
