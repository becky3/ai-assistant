...
- 配信はメッセージング・プラットフォーム抽象（`MessagingPort` の配信プリミティブ: ヘッダー / 親スレッド開始 / 記事カード / フッター）を経由する。記事の中立表現は `ArticleCard`。以下の Block Kit 描画は Slack アダプターでの実装仕様（プラットフォームごとに描画は異なる）
...
| Discord Embed 制限超過（タイトル・本文の文字数超過、画像 URL の文字数超過・不正形式） | タイトル・本文は切り詰め、画像は省略して投稿する（エラーにしない。Discord アダプター内でサニタイズ） |
| 記事リンク URL の不正（http(s) 形式でない・長すぎる） | リンクが壊れた記事は読者に価値がないため投稿せず破棄する（配信済み扱いにして以後の配信対象から外す。警告ログに記録） |
| 記事カード投稿失敗 | 該当記事をスキップし、フィードスレッドにエラー通知（記事タイトル・URL・エラー概要）を投稿して他記事・他フィードの配信を継続。失敗記事は未配信のまま残し次回の配信実行（定時配信・手動配信）で再試行する（同一実行内では再試行しない）。エラー通知自体の失敗は配信を止めない |
| 要約 LLM 失敗・空応答 | 概要（description）があればそのまま要約として使用し、無い場合のみタイトルにフォールバックする（LLM 不通でも情報量を保つ） |
