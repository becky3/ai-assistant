...
3. 取得したメッセージをロール別に変換する:
   - ボット自身のメッセージ（`user` 一致またはメッセージの `bot_id` がアプリ自身の `bot_id` と一致）→ `assistant` ロール。`bot_message` subtype のメッセージでは `user` フィールドが空になるため、`bot_id` の一致でも判定する
   - 他ユーザーのメッセージ → `user` ロール（ユーザー ID を付与して発言者を区別）
4. `bot_message` 以外のサブタイプ付きメッセージ（編集通知等）やテキストなしメッセージはスキップする
5. Block Kit メッセージ（`blocks` フィールドを持つ）は、`blocks` 内の `section` ブロックの `text` オブジェクトおよび `rich_text` ブロック内のテキスト要素（`text` / `link` タイプ）からテキストを抽出し、`text` フィールドより優先して使用する
6. トリガーメッセージ自体は履歴から除外する（チャット応答ロジック側で追加されるため）
7. 件数制限を超えた場合は古いメッセージを切り捨て、最新 N 件を使用する
...
| Block Kit メッセージ（フィード配信等） | `blocks` フィールドからテキストを抽出し、LLM に渡す |
| `bot_message` subtype のメッセージ | スキップせず処理する（ボットによる配信メッセージを含めるため） |
