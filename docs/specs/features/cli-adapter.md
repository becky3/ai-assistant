...
- 受信ファイルの取得（CSV インポート等）は送信アダプターの `read_file` 経由で行い、認証・取得手段（Slack=token 付き DL / CLI=ローカル読込）をアダプターに閉じる
...
| コンポーネント | 役割 |
| --- | --- |
| 受信リスナー抽象 | プラットフォーム接続のライフサイクル・bot identity 解決・イベント正規化（→ 共通メッセージ形式）・受信フィルタのインターフェース定義 |
| Slack 受信リスナー | Slack Bolt + Socket Mode を束ね、受信正規化を行う受信リスナー実装 |
| 送信アダプター抽象 | メッセージ送信・ファイル送信/取得（`read_file`）・スレッド履歴取得・フォーマット指示・フィード配信プリミティブ（ヘッダー / 親スレッド開始 / 記事カード / フッター）のインターフェース定義 |
...
| 共通メッセージ形式 | Slack / CLI 共通の入力メッセージ構造（中立ファイル参照を含む） |
...
| 受信ファイル取得 | token 付きダウンロード | ローカルパスから読込 |
| フィード配信 | Block Kit カードをスレッド投稿 | stdout にテキスト表示 |
