...
2. フィルタリングを実施する:
   - Bot 自身の投稿 → 無視
   - メッセージのサブタイプ（編集・削除等） → 無視。ただし `file_share`（ユーザーのファイル添付）は処理対象とし、添付ファイル付きコマンド（`feed import` 等）をメンション無しで受け付ける
   - 対象外チャンネル → 無視
...
| ファイル添付メッセージ（`file_share`） | 処理対象。コメントをコマンドとして解釈し、添付ファイルを伴う処理（CSV インポート等）を実行する。コメントが空の場合は無視 |
...
graph TD
    ...
    B -->|message| D{フィルタリング}
    D -->|Bot/subtype（file_share以外）/対象外| E[無視]
    D -->|メンション付き| E
