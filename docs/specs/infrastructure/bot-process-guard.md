...
<!-- How追加理由: Issue #833 で wmic 出力のデコード失敗を検出。外部コマンド依存を psutil に置換 -->
- プロセス生存確認・子プロセス列挙・子プロセス停止は psutil の API（`pid_exists()` / `Process.children()` / `Process.terminate()`）を使用し、外部コマンド（`tasklist` / `wmic` / `pgrep` / `taskkill` / `os.kill`）に依存しない
...
- **保護対象 PID の除外**: 呼び出し側が `exclude_pids` で渡した PID は kill 対象から除外する。bot 終了時にも存続させたい子プロセス（例: Slack 経由で起動した remote-control）を保護するために使用する
...
| 停止対象プロセスが既に消失している | kill 不要のためスキップし、PID ファイル削除のみ行う |
| 停止対象プロセスの子プロセス列挙の権限がない | ログ出力した上で本体プロセスの停止を試行する |
