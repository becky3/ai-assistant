...
| Markdownチェック (markdownlint) 実行 | Markdownファイルの構文・スタイルチェック（`docs/**/*.md`, `*.md`, `.claude/**/*.md` を対象） |
...
6. **結果の分析**
   ...
   - **markdownlint 違反あり時**: 違反ファイル、ルールコード、違反箇所、自動修正可否を抽出
...
8. **lint・型エラー・Markdown違反の詳細調査**
   ...
   - markdownlint違反箇所のMarkdownファイルを読み込み
...
10. **レポート生成**
    - テスト・lint・型チェック・Markdownチェック結果の統合サマリー
    - 失敗した場合は原因と修正案
    - カバレッジ情報（要求された場合）

**注**: pytest・ruff・mypy・markdownlintのいずれかが失敗してもプロセスを中断せず、すべてのチェックを実行して統合レポートを生成する。
...
- [ ] AC20: markdownlint を実行し、違反があれば報告する
- [ ] AC21: markdownlint の結果を pytest/ruff/mypy と統合したレポートに含める
- [ ] AC22: markdownlint の `--fix` 実行をサポートし、自動修正可能な違反を修正できる
- [ ] AC23: `.markdownlint-cli2.jsonc` の設定に従ったルールでチェックする
- [ ] AC24: markdownlint が利用できない場合（npx未インストール等）は適切にエラーを報告し、他のチェックは続行する
...
- [ ] markdownlint 実行が正しく動作するか
- [ ] pytest・ruff・mypy・markdownlintの統合レポートが正しく生成されるか
- [ ] 修正案が実用的で正確か
...
- [ ] ruff・mypy・markdownlintが利用できない場合のエラーメッセージ

### レポート品質

- [ ] 成功・失敗の判定が正確か（pytest・ruff・mypy・markdownlint それぞれ）
- [ ] スタックトレースから原因を正しく特定できるか
- [ ] lint違反・型エラー・Markdown違反から原因を正しく特定できるか
- [ ] 修正案が具体的でファイルパス・行番号を含むか
- [ ] markdownlintの `--fix` による自動修正が正しく動作するか

### 実行順序と統合レポート

- [ ] pytest → ruff → mypy → markdownlint の順で実行されるか
