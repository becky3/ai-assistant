...
2. **品質担当がcode-reviewerを使用しなかった**
   - エージェントチームの品質担当にcode-reviewer使用を指示していたが、実際には使用されなかった
   - 原因: コンテキスト圧縮後の再開時に、初期指示の一部を失念
   - 結果: テスト名のAC番号形式がCopilotレビューで指摘されるまで検出されなかった
   - 教訓: 長時間セッションでは、品質検証の手順を毎回明示的に確認する必要がある
